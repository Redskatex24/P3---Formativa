...
public class CategoriaController {
    ...
    public ResponseEntity<List<CategoriaDTO>> listar(@RequestParam(required = false) String nombre) {
        log.info("Solicitud para listar categorías. Filtro por nombre: '{}'", nombre);
        ...
        if (categoria.isEmpty()) {
            log.warn("No se encontraron categorías para la búsqueda realizada.");
    ...
    public ResponseEntity<CategoriaDTO> guardar(@Valid @RequestBody CategoriaRequest request) {
        log.info("Guardando nueva categoría con datos: {}", request);
    ...
    public ResponseEntity<CategoriaDTO> buscarPorId(@PathVariable int id){
        ...
        try{
            ...
            log.warn("No se pudo encontrar la categoría con ID: {}", id);
    ...
    public ResponseEntity<Void> eliminar(@PathVariable int id){
        log.info("Procesando la eliminación de la categoría ID: {}", id);
